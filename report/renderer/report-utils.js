...
import {Util as SharedUtils} from '../../shared/shared-utils.js';
...
class ReportUtils {
  ...
  static getEmulationDescriptions(settings) {
    ...
    switch (settings.throttlingMethod) {
      case 'provided':
        summary = networkThrottling = cpuThrottling = ReportUtils.i18n.strings.throttlingProvided;
        break;
      case 'devtools': {
        const {cpuSlowdownMultiplier, requestLatencyMs} = throttling;
        // eslint-disable-next-line max-len
        cpuThrottling = `${ReportUtils.i18n.formatNumber(cpuSlowdownMultiplier)}x slowdown (DevTools)`;
        networkThrottling = `${ReportUtils.i18n.formatMilliseconds(requestLatencyMs)} HTTP RTT, ` +
          `${ReportUtils.i18n.formatKbps(throttling.downloadThroughputKbps)} down, ` +
          `${ReportUtils.i18n.formatKbps(throttling.uploadThroughputKbps)} up (DevTools)`;
        ...
        summary = isSlow4G() ?
          ReportUtils.i18n.strings.runtimeSlow4g :
          ReportUtils.i18n.strings.runtimeCustom;
        break;
      }
      case 'simulate': {
        const {cpuSlowdownMultiplier, rttMs, throughputKbps} = throttling;
        // eslint-disable-next-line max-len
        cpuThrottling = `${ReportUtils.i18n.formatNumber(cpuSlowdownMultiplier)}x slowdown (Simulated)`;
        networkThrottling = `${ReportUtils.i18n.formatMilliseconds(rttMs)} TCP RTT, ` +
          `${ReportUtils.i18n.formatKbps(throughputKbps)} throughput (Simulated)`;
      ...
      default:
        summary = cpuThrottling = networkThrottling = ReportUtils.i18n.strings.runtimeUnknown;
    }

    // TODO(paulirish): revise Runtime Settings strings: https://github.com/GoogleChrome/lighthouse/pull/11796
    const deviceEmulation = {
      mobile: ReportUtils.i18n.strings.runtimeMobileEmulation,
      desktop: ReportUtils.i18n.strings.runtimeDesktopEmulation,
    }[settings.formFactor] || ReportUtils.i18n.strings.runtimeNoEmulation;
  ...
  static calculateCategoryFraction(category) {
    ...
    for (const auditRef of category.auditRefs) {
      const auditPassed = SharedUtils.showAsPassed(auditRef.result);
...
ReportUtils.UIStrings = UIStrings;
