...
const Util = require('../../../core/util-commonjs.js');
...
function writeStringsToCtcFiles(locale, strings) {
  const fullPath = path.join(LH_ROOT, `core/lib/i18n/locales/${locale}.ctc.json`);
  /** @type {Record<string, CtcMessage>} */
  const output = {};
...
async function main() {
  ...
  const lhl = collectAndBakeCtcStrings(path.join(LH_ROOT, 'core/lib/i18n/locales/'));
