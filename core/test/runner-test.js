...
const {LegacyNormalizedConfig} = await import('../legacy/config/config.js');
...
describe('Runner', () => {
  ...
  describe('Gather Mode & Audit Mode', () => {
    ...
    const generateConfig = settings => LegacyNormalizedConfig.fromJson({
      passes: [{
    ...
    it('does not include a top-level runtimeError when gatherers were successful', async () => {
      const config = await LegacyNormalizedConfig.fromJson({
        settings: {
    ...
    it('serializes IcuMessages in gatherMode and is able to use them in auditMode', async () => {
      ...
      const gatherConfig = await LegacyNormalizedConfig.fromJson({
      ...
      const auditConfig = await LegacyNormalizedConfig.fromJson({
  ...
  describe('lhr.runtimeError', () => {
    ...
    it('includes a top-level runtimeError when a gatherer throws one', async () => {
      const config = await LegacyNormalizedConfig.fromJson(configJson);
  ...
  it('localized errors thrown from driver', async () => {
    ...
    try {
      await runGatherAndAudit(createGatherFn('https://example.com/'), {driverMock: erroringDriver, config: await LegacyNormalizedConfig.fromJson()});
      assert.fail('should have thrown');
    } catch (err) {
