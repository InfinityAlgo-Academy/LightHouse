...
/** @typedef {import('../../../legacy/config/config.js').LegacyNormalizedConfig} LegacyNormalizedConfig */
...
async function makeConfig(json) {
  const config = await LegacyNormalizedConfig.fromJson(json);

  // Since the config is for `gather-runner`, ensure it has `passes`.
  if (!config.passes) {
    throw new Error('gather-runner test configs must have `passes`');
  }
  return /** @type {LegacyNormalizedConfig & {passes: Array<LH.Config.Pass>}} */ (config);
}
