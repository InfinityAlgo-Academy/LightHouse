...
function generateRecord({
  ...
  return {
    url,
    mimeType,
    rendererStartTime, // DevTools timestamp which is in seconds.
...
describe('OffscreenImages audit', () => {
  ...
  it('disregards images loaded after TTI', () => {
    const topLevelTasks = [{ts: 1900, duration: 100}];
    const networkRecord = generateRecord({resourceSizeInKb: 100, rendererStartTime: 3});
    const artifacts = {
      ViewportDimensions: DEFAULT_DIMENSIONS,
      GatherContext: {gatherMode: 'navigation'},
      ImageElements: [
  ...
  it('rethrow error when interactive throws error in Lantern', async () => {
    context = {settings: {throttlingMethod: 'simulate'}, computedCache: new Map()};
    const networkRecords = [
      generateRecord({url: 'a', resourceSizeInKb: 100, rendererStartTime: 3}),
      generateRecord({url: 'b', resourceSizeInKb: 100, rendererStartTime: 4}),
    ];
    const artifacts = {
      ViewportDimensions: DEFAULT_DIMENSIONS,
      GatherContext: {gatherMode: 'navigation'},
