...
async function inlineFs(code, contextPath) {
  // Basic approach:
  // - scans `code` for fs methods
  // - parses only the expression at each found index
  // - statically evaluates arguments to fs method, collapsing to single string
  // - executes fs method with computed argument
  // - replaces original expression with result of fs call
