...
jest.mock('../../../core/lib/sentry.js', () => ({init: jest.fn()}));
...
beforeEach(() => {
  ...
  initSentryFn = /** @type {*} */ (require('../../../core/lib/sentry.js').init);
...
describe('CLI bin', function() {
  ...
  describe('config', () => {
    it('should load the config from the path', async () => {
      const configPath = require.resolve('../../../core/config/lr-desktop-config.js');
    ...
    it('should load the config from the preset', async () => {
      ...
      const actualConfig = require('../../../core/config/experimental-config.js');
      await bin.begin();
  ...
  describe('budget', () => {
    it('should load the config from the path', async () => {
      const budgetPath = '../../../core/test/fixtures/simple-budget.json';
