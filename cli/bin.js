...
import {getConfigDisplayString, initializeConfig} from '../core/config/config.js';
import {LegacyNormalizedConfig} from '../core/legacy/config/config.js';
...
async function begin() {
  ...
  if (cliFlags.printConfig) {
    if (cliFlags.legacyNavigation) {
      const config = await LegacyNormalizedConfig.fromJson(configJson, cliFlags);
      process.stdout.write(config.getPrintString());
    } else {
      const {config} = await initializeConfig('navigation', configJson, cliFlags);
