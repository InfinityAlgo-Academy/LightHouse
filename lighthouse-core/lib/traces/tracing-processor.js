...
class TraceProcessor {
  ...
  static findTracingIds(events) {
    ...
    if (startedInBrowserEvt && startedInBrowserEvt.args.data &&
      ...
      const frameCommitedEvt = frameId ?
        events.find(e => Boolean(
          e.name === 'FrameCommittedInBrowser' && e.args &&
          e.args.data && e.args.data.frame === frameId))
        : undefined;
      pid = frameCommitedEvt && frameCommitedEvt.args.data && frameCommitedEvt.args.data.processId;

      // if loading about:blank, FrameCommittedInBrowser will not be emitted
      if (!pid) {
