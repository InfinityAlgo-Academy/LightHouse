...
describe('ProtocolSession', () => {
  ...
  beforeEach(() => {
    ...
    puppeteerSession = {emit: jestMock.fn(), send: jestMock.fn().mockResolvedValue()};
  ...
  describe('ProtocolSession', () => {
    it('should emit a copy of events on "*"', () => {
      ...
      const regularListener = jestMock.fn();
      const allListener = jestMock.fn();
  ...
  for (const method of delegateMethods) {
    describe(`.${method}`, () => {
      it('delegates to puppeteer', async () => {
        const puppeteerFn = puppeteerSession[method] = jestMock.fn();
  ...
  describe('.dispose', () => {
    it('should detach from the session', async () => {
      const detach = jestMock.fn();
      const removeAllListeners = jestMock.fn();
      // @ts-expect-error - we want to use a more limited test.
      puppeteerSession = {detach, emit: jestMock.fn(), removeAllListeners};
      session = new ProtocolSession(puppeteerSession);
  ...
  describe('.addSessionAttachedListener', () => {
    it('should listen for new sessions', () => {
      const mockOn = jestMock.fn();
      // @ts-expect-error - we want to use a more limited, controllable test
      puppeteerSession = {connection: () => ({on: mockOn}), emit: jestMock.fn()};
      session = new ProtocolSession(puppeteerSession);
      ...
      mockOn.mock.calls[0][1]({emit: jestMock.fn()});
  ...
  describe('.removeSessionAttachedListener', () => {
    it('should stop listening for new sessions', () => {
      const mockOn = jestMock.fn();
      const mockOff = jestMock.fn();
      // @ts-expect-error - we want to use a more limited, controllable test
      puppeteerSession = {connection: () => ({on: mockOn, off: mockOff}), emit: jestMock.fn()};
      session = new ProtocolSession(puppeteerSession);

      // Make sure we listen for the event.
      const userListener = jestMock.fn();
  ...
  describe('.sendCommand', () => {
    it('delegates to puppeteer', async () => {
      const send = puppeteerSession.send = jestMock.fn().mockResolvedValue(123);
    ...
    it('times out a request by default', async () => {
      ...
      puppeteerSession.send = jestMock.fn().mockReturnValue(sendPromise.promise);
