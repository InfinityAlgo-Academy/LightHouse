...
describe('Runner', () => {
  ...
  describe('Gather Mode & Audit Mode', () => {
    ...
    const generateConfig = settings => Config.createConfigFromJson({
      passes: [{
    ...
    it('serializes IcuMessages in gatherMode and is able to use them in auditMode', async () => {
      ...
      const gatherConfig = await Config.createConfigFromJson({
      ...
      const auditConfig = await Config.createConfigFromJson({
  ...
  it('localized errors thrown from driver', async () => {
    ...
    try {
      await runGatherAndAudit(createGatherFn('https://example.com/'), {driverMock: erroringDriver, config: await Config.createConfigFromJson()});
      assert.fail('should have thrown');
    } catch (err) {
