...
assert(Runner._runAudit.mock);

const moduleDir = getModuleDirectory(import.meta);

// @ts-expect-error
const {lighthouseTestContext} = global;
  ...
  describe('Gather Mode & Audit Mode', () => {
    ...
    it('-G gathers, quits, and doesn\'t run audits', async () => {
      ...
      return runGatherAndAudit(createGatherFn(url), opts).then(_ => {
        expect(lighthouseTestContext.loadArtifactsSpy).not.toHaveBeenCalled();
        expect(lighthouseTestContext.saveArtifactsSpy).toHaveBeenCalled();

        const saveArtifactArg = lighthouseTestContext.saveArtifactsSpy.mock.calls[0][0];
        assert.ok(saveArtifactArg.ViewportDimensions);
        assert.ok(saveArtifactArg.devtoolsLogs.defaultPass.length > 100);

        expect(lighthouseTestContext.gatherRunnerRunSpy).toHaveBeenCalled();
        expect(lighthouseTestContext.runAuditSpy).not.toHaveBeenCalled();
        expect(lighthouseTestContext.saveLhrSpy).not.toHaveBeenCalled();
    ...
    it('-A audits from saved artifacts and doesn\'t gather', async () => {
      ...
      return runGatherAndAudit(createGatherFn(), opts).then(_ => {
        expect(lighthouseTestContext.loadArtifactsSpy).toHaveBeenCalled();
        expect(lighthouseTestContext.gatherRunnerRunSpy).not.toHaveBeenCalled();
        expect(lighthouseTestContext.saveArtifactsSpy).not.toHaveBeenCalled();
        expect(lighthouseTestContext.saveLhrSpy).not.toHaveBeenCalled();
        expect(lighthouseTestContext.runAuditSpy).toHaveBeenCalled();
      });
    });
    ...
    it('-GA is a normal run but it saves artifacts and LHR to disk', async () => {
      ...
      return runGatherAndAudit(createGatherFn(url), opts).then(_ => {
        ...
        expect(lighthouseTestContext.gatherRunnerRunSpy).toHaveBeenCalled();
        expect(lighthouseTestContext.saveArtifactsSpy).toHaveBeenCalled();
        expect(lighthouseTestContext.saveLhrSpy).toHaveBeenCalled();
        expect(lighthouseTestContext.runAuditSpy).toHaveBeenCalled();
      });
    });
